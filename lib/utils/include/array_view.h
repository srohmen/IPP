...
#define CONSTEXPR_FALLBACK constexpr
#else
#define CONSTEXPR_FALLBACK
#endif
...
public:
    ...
    CONSTEXPR_FALLBACK offset(value_type v) noexcept { (*this)[0] = v; }
    CONSTEXPR_FALLBACK offset(std::initializer_list<value_type> il);

    // element access
    CONSTEXPR_FALLBACK reference       operator[](size_type n) { return offset_[n]; }
...
template <size_t Rank>
CONSTEXPR_FALLBACK offset<Rank>::offset(std::initializer_list<value_type> il)
{
...
// offset equality
template <size_t Rank>
CONSTEXPR_FALLBACK bool operator==(const offset<Rank>& lhs, const offset<Rank>& rhs) noexcept
{
...
public:
    ...
    template <size_t R = Rank, typename = std::enable_if_t<R == 1>>
    CONSTEXPR_FALLBACK bounds(value_type v)
    {
        (*this)[0] = v; postcondition();
    }

    CONSTEXPR_FALLBACK bounds(std::initializer_list<value_type> il);

    // observers
    CONSTEXPR_FALLBACK size_type size() const noexcept;
    CONSTEXPR_FALLBACK bool      contains(const offset<Rank>& idx) const noexcept;
    ...
    {
        iterator iter{*this};
        return iter.setOffTheEnd();
    }

    // element access
    CONSTEXPR_FALLBACK reference operator[](size_type n) { return bounds_[n]; }
...
// construction
template <size_t Rank>
CONSTEXPR_FALLBACK bounds<Rank>::bounds(const std::initializer_list<value_type> il)
{
    assert(il.size() == Rank);
...
// observers
template <size_t Rank>
CONSTEXPR_FALLBACK size_t bounds<Rank>::size() const noexcept
{
    size_type product{1};
...
template <size_t Rank>
CONSTEXPR_FALLBACK bool bounds<Rank>::contains(const offset<Rank>& idx) const noexcept
{
    for (size_type i=0; i<Rank; ++i) {
...
template<size_t Rank>
bounds_iterator<Rank> &bounds_iterator<Rank>::setOffTheEnd()
{
    if(bounds_.size() == 0)
    {
...
CONSTEXPR_FALLBACK T& view_access(T* data, const offset<Rank>& idx, const offset<Rank>& stride)
{
    std::ptrdiff_t off{};
    for (size_t i=0; i<Rank; ++i)
    {
...
public:
    ...
    CONSTEXPR_FALLBACK array_view(Viewable&& vw, bounds_type bounds)
    ...
    CONSTEXPR_FALLBACK offset_type stride() const noexcept;
    constexpr pointer     data()   const noexcept { return data_; }

    CONSTEXPR_FALLBACK reference operator[](const offset_type& idx) const
    {
    ...
    CONSTEXPR_FALLBACK array_view<T, Rank-1> operator[](std::ptrdiff_t slice) const
    {
...
template <typename T, size_t Rank>
CONSTEXPR_FALLBACK typename array_view<T,Rank>::offset_type array_view<T,Rank>::stride() const noexcept
{
    offset_type stride{};
    stride[rank-1] = 1;
...
struct MakeBounds<2>
{
    static av::bounds<2> get(size_t a, size_t b, size_t /*c*/)
    {
        av::bounds<2> bounds = {
    ...
    static av::bounds<3> get(size_t a, size_t b, size_t c, size_t /*d*/)
    {
        av::bounds<3> bounds = {
            static_cast<std::ptrdiff_t>(a),
            static_cast<std::ptrdiff_t>(b),
...
struct MakeBounds<3>
{
    static av::bounds<3> get(size_t a, size_t b, size_t c)
    {
        av::bounds<3> bounds = {
            static_cast<std::ptrdiff_t>(a),
            static_cast<std::ptrdiff_t>(b),
    ...
    static av::bounds<4> get(size_t a, size_t b, size_t c, size_t d)
    {
        av::bounds<4> bounds = {
            static_cast<std::ptrdiff_t>(a),
            static_cast<std::ptrdiff_t>(b),
